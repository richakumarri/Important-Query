...
create  table scratch.riders.last_180_dys_metrics as
 ...
 ,master_table as
    ...
    *, row_number () over (partition by zone_code, city_name, order_month order by order_cnt desc)as rnk
    ...
    group by 1,2,3
     ...
     ,avg(RET)as avg_RET_last_180_dys
     ,PERCENTILE_CONT( 0.1) WITHIN GROUP (ORDER BY RET)as p10_RET_last_180_dys
     ,PERCENTILE_CONT( 0.2) WITHIN GROUP (ORDER BY RET)as p20_RET_last_180_dys
     ,PERCENTILE_CONT( 0.5) WITHIN GROUP (ORDER BY RET)as p50_RET_last_180_dys
     ,PERCENTILE_CONT( 0.7) WITHIN GROUP (ORDER BY RET)as p70_RET_last_180_dys
     ,PERCENTILE_CONT( 0.9) WITHIN GROUP (ORDER BY RET)as p90_RET_last_180_dys
